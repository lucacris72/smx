...
class instr_trace_t;
  ...
  function void push_reg_read(input logic [5:0] addr, input logic [31:0] value, input bit filled);
    reg_t tmp;
    tmp.addr   = addr;
    tmp.value  = value;
    tmp.filled = filled;
    regs_read.push_back(tmp);
  endfunction

  function void push_reg_write(input logic [5:0] addr, input logic [31:0] value, input bit filled);
    reg_t tmp;
    tmp.addr   = addr;
    tmp.value  = value;
    tmp.filled = filled;
    regs_write.push_back(tmp);
  endfunction
  ...
  function void printRInstr(input string mnemonic);
    begin
      ...
      push_reg_read(rs1, rs1_value, 0);
      push_reg_read(rs2, rs2_value, 0);
      push_reg_write(rd, 'x, 0);
  ...
  function void printMulInstr();
    ...
    begin
      ...
      if (instr[13]) push_reg_read(rd, rs3_value, 0);
    ...
    begin
      ...
      if (mnemonic == "cv.cplxconj") begin
        ...
        if (str_sci == "") begin
          ...
          push_reg_read(rs2, rs2_value_vec, 0);
          str_args = $sformatf("%s, %s, %s", regAddrToStr(rd), regAddrToStr(rs1), regAddrToStr(rs2));
